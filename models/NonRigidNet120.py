...
class NonRigidNet120(nn.Module):
    def __init__(self, mod = 'NonRigidTransform', num_frames = 100, num_joints = 25):
        ...
        if mod == 'NonRigidTransform':
            self.rot = NonRigidTransform(num_frames,num_joints)
        elif mod == 'NonRigidTransformInit':
            self.rot = NonRigidTransformInit(num_frames,num_joints)
    ...
    def forward(self, x):
        ...
        # INTERPOLATION LAYER
        x = F.interpolate(x, scale_factor=10, mode='linear')  # change scale_fator
        x = self.conv1(x)
