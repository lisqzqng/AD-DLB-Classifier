...
parser.add_argument('--layer_name', default='RigidTransform', type=str, help='RigidTransform, RigidTransformInit, NonRigidTransform, NonRigidTransformInit')
...
if opt.layer_name not in options:
    print('Wrong layer name, will be using default RigidTransform')
    mod = options[0]
else:
    mod = opt.layer_name
...
if mod == 'RigidTransform' or mod == 'RigidTransformInit':
    rigid = True
else:
    rigid = False
        
if rigid == True:
    model = RigidNet60(mod,num_frames,num_joints).to(device)
else:
    model = NonRigidNet60(mod,num_frames,num_joints).to(device)
...
if save_model == True:
    torch.save(model, './models/checkpoints/model_onerun_{}_{}_NTU{}_{}.pth'.format(prot,mod,num_labels,cast))
